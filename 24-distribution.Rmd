...
```{r distributions-load-data, eval=FALSE}
load("data/bdims.RData")
```

Let's take a quick peek at the first few rows of the data.

```{r distributions-head-data, eval=FALSE}
head(bdims)
```
...
```{r distributions-male-female, eval=FALSE}
...
```{r distributions-female-hgt-mean-sd, eval=FALSE}
...
```{r distributions-hist-height, eval=FALSE}
...
```{r distributions-sim-norm, eval=FALSE}
...
```{r distributions-qqnormsim, eval=FALSE}
qqnormsim(fdims$hgt)
```
...
```{r distributions-pnorm, eval=FALSE}
...
```{r distributions-probability, eval=FALSE}
